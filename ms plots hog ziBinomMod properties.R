...
plot_dir <- "C:/Users/Abigail.Feuka/OneDrive - USDA/Feral Hogs/Missouri/nimble/Model outputs/Plots/Properties"
##study site--------------------
...
# load samples ---------------------
load("C:/Users/Abigail.Feuka/OneDrive - USDA/Feral Hogs/Missouri/nimble/Model outputs/ziBinMod_area_logit_det_20_24_prop_count_12DEC24.Rdata")
...
# }
subfolder <- "Summarized to Counties"

## removal data -----------
dat_rem <- rbind.data.frame(dat_aerial,dat_trap)
dat_rem_sum <- dat_rem %>% 
  group_by(prop_rem_idx,SiteID,Method,month=floor_date(Date,"month")) %>% 
...
beta<- cbind.data.frame(do.call("rbind",lapply(1:nChains,function(i){
  cbind.data.frame(samples[[i]][grepl("beta",colnames(samples[[i]])) & 
                                  ...
                                  !grepl("beta0",colnames(samples[[i]]))],
                   chain=i)})),
...
ggsave(g_b, 
       filename = paste0(plot_dir,"/",subfolder,"/Trace/beta_trace.jpeg"),
       width=7,height=5,units="in",device="jpeg")

##seasonal random effects----------------------
beta0<- cbind.data.frame(do.call("rbind",lapply(1:nChains,function(i){
  cbind.data.frame(samples[[i]][grepl("beta0",colnames(samples[[i]]))],
                   chain=i)})),
  samp=rep(1:nmcmc,nChains))

beta0_long <- beta0 %>% 
  pivot_longer(cols=all_of(1:nperiods),names_to="beta",values_to="value") %>% 
  mutate(chain=factor(chain))

g_b0<- ggplot(beta0_long) + geom_line(aes(x=samp,y=value,col=chain))+
  geom_hline(yintercept=0,col="black",lty=2)+
  facet_wrap(.~beta)
g_b0

ggsave(g_b0, 
       filename = paste0(plot_dir,"/",subfolder,"/Trace/beta0_trace.jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
beta_lam_names <- c("Intercept","NFSP")
colnames(beta_lam)[1:2] <- beta_lam_names

beta_lam_long <- beta_lam %>% 
  pivot_longer(cols=all_of(1:2),names_to="beta",values_to="value") %>% 
  mutate(chain=factor(chain))
...
ggsave(g_bl, 
       filename = paste0(plot_dir,"/",subfolder,"/Trace/beta_lam_trace.jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
ggsave(g_n1, 
       filename = paste0(plot_dir,"/",subfolder,"/Trace/n1_params_trace.jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
ggsave(g_alpha, 
       filename = paste0(plot_dir,"/",subfolder,"/Trace/detection_trace.jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
lambda_long <- lambda %>% 
  pivot_longer(cols=all_of(1:ncounties*nperiods),names_to="site_per",values_to="value") %>% 
  mutate(chain=factor(chain))
...
ggsave(g_l, 
       filename = paste0(plot_dir,"/",subfolder,"/Trace/lambda_trace.jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/pabs_all_yrs.jpeg"),
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/km2_eliminated_",elim_thresh,".jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
ggplot(pabs_thresh_ext)+ 
  ...
  theme(text=element_text(size=15))
ggsave(filename=paste0(plot_dir,"/",subfolder,"/mi2_eliminated_",elim_thresh,".jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/prop_eliminated_",elim_thresh,".jpeg"),
       width=7,height=5,units="in",device="jpeg")
...
#   theme(text=element_text(size=15))
# 
# ggsave(filename=paste0("./Model outputs/Plots/",subfolder,"/prop_area_eliminated_",elim_thresh,"_ea.jpeg"),
#        width=7,height=5,units="in",device="jpeg")
...
write.csv(pabs_thresh_yr,
          paste0(plot_dir,"/",subfolder,"/area_above_,",elim_thresh,"_table.csv"))
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/occ_det_eff_curve.jpeg"),
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/pelim_all_yrs.jpeg"),
       device="jpeg",width=8,height=10,units="in")
...
ggsave(filename = paste0(plot_dir,"/",subfolder,"/nweeks_to_elim_trapeff.jpeg"),
       width=7,height=5,units="in",device="jpeg")

#abundance  -----------------------------
## properties -----------------
N <- cbind.data.frame(do.call("rbind",lapply(1:nChains,function(i){
  cbind.data.frame(samples[[i]][grepl("N_prop",colnames(samples[[i]]))],
                   chain=i)})),
  samp=rep(1:nmcmc,nChains))
...
N_long$prop_idx <- rep(rep(1:nproperties,nperiods),nmcmc*nChains)
N_long$period_idx <- rep(sort(rep(1:nperiods,nproperties)),nmcmc*nChains)
N_long <- N_long %>%
  left_join(site_idx_lookup %>% rename(prop_idx=prop_rem_idx))
...
N_long <- N_long %>% left_join(county_idx %>% rename(prop_idx=prop_rem_idx))

# lambda_N <- lambda
# lambda_N <- lambda_N %>% 
#   pivot_longer(cols=grep("lambda",colnames(lambda_N)),
#                names_to="idx",
#                values_to="lambda")

# lambda_N$site_idx <- sapply(1:nrow(lambda_N),function(x){
#   as.numeric(gsub("\\[","",gsub("lambda","",unlist(str_split(lambda_N[x,"idx"],","))[1])))})
# 
# lambda_N$period_idx <- sapply(1:nrow(lambda_N),function(x){
#   as.numeric(gsub("\\]","",unlist(str_split((lambda_N[x,"idx"]),","))[2]))})
# 
# lambda_N$per_start <- sapply(1:nrow(lambda_N),function(i){
#   min(dat_occ$per_start[dat_occ$period_idx==lambda_N$period_idx[i]])})
# lambda_N$per_start <- as.Date(lambda_N$per_start,origin='1970-01-01')
# lambda_N$year <- year(lambda_N$per_start)
# lambda_N$fy <- lambda_N$year
# lambda_N$fy[month(lambda_N$per_start)%in%c(10:12)] <- 
#   lambda_N$fy[month(lambda_N$per_start)%in%c(10:12)] +1
# 
...
#density 
N_long <- N_long %>% mutate(dens=N/prop_area_km
                            # dens_no_rem=N_no_rem/prop_area_km,
                            # N_ss_no_rem=N_no_rem/N_ss
                            )

N_sum <- N_long %>% 
  pivot_longer(cols=c("N","N_std","dens"
                      # "N_no_rem","dens_no_rem","N_ss_no_rem",
                      # "lambda_prop","lambda_prop_std","lambda_prop_dens"
                      ),
               names_to="metric",values_to="value") %>% 
  group_by(metric,prop_idx,period_idx,per_start,year,fy) %>% 
  summarise(mn=mean(value),
            md=median(value),
            lci=quantile(value,0.025),
            uci=quantile(value,0.975)) %>% 
...
####raw abundance-------------------------
dat_rem_sum <- dat_rem_sum %>% left_join(county_idx)
dat_rem_sum$tot_rem_km <- dat_rem_sum$removal/dat_rem_sum$prop_area_km

site_rem <- dat_rem %>%
  group_by(Date,prop_rem_idx) %>%
  summarise(tot_rem=sum(tot_rem)) %>%
  filter(tot_rem>0) %>%
  group_by(prop_rem_idx) %>%
  summarise(n=n()) %>%
  arrange(desc(n))
...
N_top <- N_sum %>% 
  filter(prop_idx%in%site_rem_cutoff$prop_rem_idx     )

dat_rem_sum_top <- dat_rem_sum %>% 
  filter(prop_rem_idx%in%site_rem_cutoff$prop_rem_idx     )

axis_trans<- 0.1
ggplot()+  
  geom_ribbon(data=N_top,
              aes(x=per_start,ymin=lci_N,ymax=uci_N,fill="Estimated Abundance"),alpha=0.2)+
  geom_line(data=N_top,
  ...
  #                 fill="Removal Needed"),
  #             alpha=0.1)+
  # geom_line(data=N_top,aes(x=per_start,y=md_lambda_prop/axis_trans,group=SiteID,
  #                          col="Removal Needed"),
  #           lwd=1,lty=2)+
  # geom_ribbon(data=N_top,
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/abundance_trend_removal.jpeg"),
       device="jpeg",width=10,height=6,units="in")
...
ggplot()+  
  ...
  #                 fill="Removal Needed"),
  #             alpha=0.1)+
  # geom_line(data=N_top,aes(x=per_start,
  ...
  # guides(color="none")+
  theme(text=element_text(size=15))
ggsave(filename=paste0(plot_dir,"/",subfolder,"/density_trend_removal.jpeg"),
       device="jpeg",width=10,height=6,units="in")

# ggsave(filename=paste0(plot_dir,"/density_trend_removal_no_red.jpeg"),
#        device="jpeg",width=10,height=6,units="in")
...
ggplot()+  
  ...
  #                          group=SiteID,col="Removal Needed"),
  #           lwd=1,lty=2)+
  # geom_ribbon(data=N_top,
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/std_abundance_trend_removal.jpeg"),
       device="jpeg",width=10,height=6,units="in")

## counties -----------------
N_site <- cbind.data.frame(do.call("rbind",lapply(1:nChains,function(i){
  cbind.data.frame(samples[[i]][grepl("N_site",colnames(samples[[i]]))],
                   chain=i)})),
  samp=rep(1:nmcmc,nChains))

N_site_long <- N_site %>% pivot_longer(cols=1:(ncol(N_site)-2),
                             values_to = "N_site",
                             names_to="idx") %>% 
  select(-idx)

# N_site_long$elim_area_idx <- rep(rep(c(5,7),nperiods),nmcmc*nChains)
N_site_long$site_idx <- rep(rep(1:ncounties,nperiods),nmcmc*nChains)
N_site_long$period_idx <- rep(sort(rep(1:nperiods,ncounties)),nmcmc*nChains)

N_site_long <- N_site_long %>%
  left_join(county_idx %>% 
              rename(site_idx=county_idx) %>% 
              select(site_idx,county_area_km) %>% 
              distinct())
...
N_site_long <- N_site_long %>% left_join(per_idx)
...
#standardizes abundance 
N_site_ss <- sqrt(sum(N_site_long$N_site^2)/(length(N_site_long$N_site)-1))
N_site_long$N_site_std <- N_site_long$N_site/N_site_ss
# N_site_long$lambda_prop_std <- N_site_long$lambda_prop/N_site_ss

#projecting pop growth sans removal 
# N_site_long$N_site_no_rem <- NA
# N_site_long$N_site_no_rem[N_site_long$period_idx==1] <- N_site_long$N_site[N_site_long$period_idx==1] 
# for(i in 2:nperiods){
#   N_site_long$N_site_no_rem[N_site_long$period_idx==i] <-
#     rpois(length(N_site_long$N_site_no_rem[N_site_long$period_idx==i]),
#           N_site_long$N_site_no_rem[N_site_long$period_idx==(i-1)]*N_site_long$lambda[N_site_long$period_idx==(i-1)])
# }

#density 
N_site_long <- N_site_long %>% mutate(dens=N_site/county_area_km
                            # dens_no_rem=N_no_rem/prop_area_km,
                            # N_ss_no_rem=N_no_rem/N_ss
)

N_site_sum <- N_site_long %>% 
  pivot_longer(cols=c("N_site","N_site_std","dens"
                      # "N_site_no_rem","dens_no_rem","N_site_ss_no_rem",
                      # "lambda_prop","lambda_prop_std","lambda_prop_dens"
  ),
  names_to="metric",values_to="value") %>% 
  group_by(metric,site_idx,period_idx,per_start,year,fy) %>% 
  summarise(mn=mean(value),
            md=median(value),
            lci=quantile(value,0.025),
            uci=quantile(value,0.975)) %>% 
  pivot_wider(names_from=metric,values_from = c(mn,md,lci,uci))
N_site_sum <- N_site_sum %>% left_join(county_idx %>% 
                           select(county_idx,SiteID) %>% 
                           rename(site_idx=county_idx) %>% 
                           distinct()) %>% 
  left_join(study_site_grid %>% st_drop_geometry() %>% 
              select(SiteID,COUNTY))

### annual ------------------------
# N_site_yr <- N_site_long %>% filter(month(per_start)==10 | (month(per_start)==9 & year(per_start)==2024))
# 
# N_site_yr$per_temp <- as.numeric(as.factor(N_site_yr$period_idx))
# N_site_yr <- N_site_yr %>% filter(SiteID%in%site_rem_cutoff$SiteID)
# 
# N_site_yr$N_site_prev<- NA
# for(i in 1:nrow(N_site_yr)){
#   if(N_site_yr$per_temp[i]>1){
#     N_site_yr$N_site_prev[i] <- N_site_yr$N_site[N_site_yr$per_temp==(N_site_yr$per_temp[i]-1) &
#                                        N_site_yr$SiteID==N_site_yr$SiteID[i] &
#                                        N_site_yr$samp==N_site_yr$samp[i] &
#                                        N_site_yr$chain==N_site_yr$chain[i]]
#   }
# }
# 
# N_site_yr$per_change <- (N_site_yr$N_site-N_site_yr$N_site_prev)/N_site_yr$N_site_prev
# 
# N_site_yr_sum <- N_site_yr %>% 
#   filter(year>2020) %>% 
...
# 
# write.csv(N_site_yr_sum,paste0("C:/Users/Abigail.Feuka/OneDrive - USDA/Feral Hogs/Missouri/nimble/Model outputs/Plots/",subfolder,"/N_site_change_table.csv"))

####raw abundance-------------------------
dat_rem_sum_site <- dat_rem_sum %>% 
  group_by(county_idx,Method,month) %>% 
  summarise(removal=sum(removal),
            tot_hrs=sum(tot_hrs),
            county_area_km=unique(county_area_km))
dat_rem_sum_site$tot_rem_km <- dat_rem_sum_site$removal/dat_rem_sum_site$county_area_km

axis_trans<- 0.1
ggplot()+  
  geom_ribbon(data=N_site_sum,
              aes(x=per_start,ymin=lci_N_site,ymax=uci_N_site,
                  fill="Estimated Abundance"),alpha=0.2)+
  geom_line(data=N_site_sum,
            aes(x=per_start,y=md_N_site,col="Estimated Abundance"),lwd=1)+
  geom_bar(data=dat_rem_sum_site,
           aes(x=month,y=removal/axis_trans),stat="identity",
           alpha=0.6)+
  # geom_ribbon(data=N_site_sum,
  #             aes(x=per_start,
  ...
  #                 fill="Removal Needed"),
  #             alpha=0.1)+
  # geom_line(data=N_site_sum,aes(x=per_start,y=md_lambda_prop/axis_trans,group=SiteID,
  #                          col="Removal Needed"),
  #           lwd=1,lty=2)+
  # geom_ribbon(data=N_site_sum,
  #             aes(x=per_start,ymin=lci_N_site_no_rem,ymax=uci_N_site_no_rem,
  #                 fill="No Removal"),
#             alpha=0.1)+
# geom_line(data=N_site_sum,
#           aes(x=per_start,y=md_N_site_no_rem,col="No Removal"),
#           lwd=1,lty=3)+
facet_wrap(.~COUNTY)+
  scale_y_continuous(sec.axis = sec_axis(trans=~.*axis_trans, name="No. feral swine removed"))+
  # scale_fill_manual(name="",
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/abundance_trend_removal_county.jpeg"),
       device="jpeg",width=10,height=6,units="in")
...
ggplot()+  
  ...
  #                 fill="Removal Needed"),
  #             alpha=0.1)+
  # geom_line(data=N_site_sum,aes(x=per_start,
  #                          y=md_lambda_prop_dens/axis_trans_d,group=SiteID,
#                          col="Removal Needed"),lwd=1,lty=2)+
# geom_ribbon(data=N_site_sum,
#             aes(x=per_start,ymin=lci_dens_no_rem,ymax=uci_dens_no_rem,fill="No Removal"),
...
facet_wrap(.~COUNTY)+
  ...
  # guides(color="none")+
  theme(text=element_text(size=15))
ggsave(filename=paste0(plot_dir,"/",subfolder,"/density_trend_removal_county.jpeg"),
       device="jpeg",width=10,height=6,units="in")
...
ggplot()+  
  geom_ribbon(data=N_site_sum,
              aes(x=per_start,ymin=lci_N_site_std,ymax=uci_N_site_std,
                  fill="Estimated \nStandardized Abundance"),alpha=0.2)+
  geom_line(data=N_site_sum,
            aes(x=per_start,y=md_N_site_std,col="Estimated \nStandardized Abundance"),lwd=1)+
  geom_bar(data=dat_rem_sum_site,
           aes(x=month,y=removal/axis_scale),stat="identity",
           alpha=0.6)+
  # geom_ribbon(data=N_site_sum,
  ...
  #           lwd=1,lty=2)+
  # geom_ribbon(data=N_site_sum,
#             aes(x=per_start,ymin=lci_N_site_ss_no_rem,ymax=uci_N_site_ss_no_rem,
#                 fill="No Removal"),
#             alpha=0.1)+
# geom_line(data=N_site_sum,
#           aes(x=per_start,y=md_N_site_ss_no_rem,col="No Removal"),lty=3,lwd=1)+
facet_wrap(.~COUNTY)+
  scale_y_continuous(sec.axis = sec_axis(trans=~.*axis_scale, name="No. feral swine removed"))+
  # scale_fill_manual(name="",
  #                   values=c("Estimated \nStandardized Abundance"="black",
...
ggsave(filename=paste0(plot_dir,"/",subfolder,"/std_abundance_trend_removal_county.jpeg"),
       device="jpeg",width=10,height=6,units="in")
...
aerial_area_mn <- mean(dat_aerial$prop_property_impact[dat_aerial$prop_property_impact!=0])
...
trap_area_mn <- mean(dat_trap$prop_property_impact[dat_trap$prop_property_impact!=0])
...
ggsave(g_all,file=paste0(plot_dir,"/",subfolder,"/rem_det_curve_all.jpeg"),
       device="jpeg",width=13,height=6,units="in")

##removal comparisons --------------------
a_mn <- max(dat_aerial$prop_property_impact)*
...
max(dat_aerial$prop_property_impact)

t_mn <- max(dat_trap$prop_property_impact)*
...
max(dat_trap$prop_property_impact)
...
lam_sum <- data.frame(mn=colMeans(lambda[,grepl("lambda",colnames(lambda))]),
...
save(eff_elim_sum,N_sum,N_yr_sum,pabs_sum_ea,pabs_sum,
     ...
     pelim_ea,pelim_ea_fy,
     pelim_sum,pelim_sum_fy,
     det_a_sum,det_t_sum,
     pabs_thresh_yr,det_sum,nsites_rem,nsites_occ,nperiods,nmcmc,nChains,
     dat_occ,dat_rem,dat_rem_sum,elim_thresh,rem_df,
     file=paste0("C:/Users/Abigail.Feuka/OneDrive - USDA/Feral Hogs/Missouri/nimble/Model outputs/Plots/",subfolder,"/posterior_summaries_04NOV24.RData"))
