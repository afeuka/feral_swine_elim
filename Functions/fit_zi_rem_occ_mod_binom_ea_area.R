...
fit_zi_rem_occ <- function(sysbait_det_eff, #output of data_functions_ws_occ_ea_rem.R
  ...
  ZIbinomcode <- nimbleCode({
    ...
    p_n1 ~ dbeta(0.1,5)#dunif(0.0001,0.001)
    r_n1 ~ dgamma(6,1) #dunif(1,5)

    # lambda ~ dunif(0.7,1.3)
    mu_lam ~ dnorm(0,0.15)
    sd_lam ~ dgamma(1,20)
    # tau_sa ~ dgamma(10,10)
    ...
    for(k in 1:nsites){ #watershed loop
      for(t in 1:nperiods){
        logit(psi[k,t]) <- beta_season[t] + #beta[2]*nfsp[k,t] + 
          beta[2]*develop[k] + beta[3]*agri[k] #+ sa[k]
