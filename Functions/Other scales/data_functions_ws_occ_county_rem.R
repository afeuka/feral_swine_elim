...
grid_removals <- function(study_site_grid,
  ...
  rem_site <- rem_site %>% 
    left_join(study_site_grid %>% 
                ...
                st_drop_geometry()) %>% 
    select(Date,SiteID,Total,Method,section,site_area_km,ea_area_km) %>% 
...
grid_effort <- function(sysbait_det_eff,#output from grid_sysbaittake,by subperiod
  ...
  rem_day <-
    rem_site %>%  
    group_by(Date=floor_date(Date,"day"),period,Method,SiteID) %>% 
  ...
  sysbait_det_eff <- sysbait_det_eff %>% 
    filter(subper_start>=min(rem_day$Date) &
             subper_end<=max(rem_day$Date))%>% 
    st_drop_geometry() %>% 
  ...
  eff_day <- eff_day %>%
    left_join(ao %>% rename(Date=day,
                ...
                select(SiteID,Date,method,flight_time,effect_area_km,prop_ea_impact))
  ...
  rem_day <- rem_day %>% rename(method=Method)
  rem_day$method[rem_day$method=="Ground Shoot"] <- "Ground"

  rem_eff_ea <- eff_day_site %>% 
    filter(Date>=min(rem_day$Date) & 
             Date<=max(rem_day$Date)) %>% 
    left_join(rem_day) 
